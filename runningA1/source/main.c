/*
    ...
    Author notes:
        ...
        I am really sorry. This version is really disorganized both
        in code and in how files are stored. Next version will be adding
        better code readability and file structure, along with new features 
        such as background music, intro, death, and (if time allows) a shop 
        where you can spend coins at.
...
#include <nds/dma.h>
#include "player.h" // grit [file].png -gB8 -gt -gT 000000 (transparency)
#include "enemy.h"
#include "sword.h"
#include "coin.h"
#include "heart.h"
#include "grass.h"
#include "platform.h"
#include "bg.h" // grit [file].png -gB8 -gt -gT! (no transparency)
#include "pickup_bin.h"
#include "jump_bin.h"
#include "slash_bin.h"
#include "hit_bin.h"

int onCollision(SpriteEntry* sprite1, int sprite1W, int sprite1H, SpriteEntry* sprite2, int sprite2W, int sprite2H){
...
void updateLives(int lives, SpriteEntry* hearts[]){
...
const int freq = 44100; // Frequency used for tracks

int main(){
    srand(time(NULL));
    char* FILE = "nitro:/runningA1.nds";
    char** NAME = &FILE;
    nitroFSInit(NAME);
    soundEnable();
    ...
    while(1){
        ...
        if((keysDown() & KEY_A) && !isJumping){
            soundPlaySample(jump_bin, SoundFormat_16Bit, jump_bin_size, freq, 64, 64, false, 0);;
            player->y -= 5;
            yvel = -11;
        }
            
        if((keysHeld() & KEY_B) && !isSwordOut){
            soundPlaySample(slash_bin, SoundFormat_16Bit, slash_bin_size, freq, 64, 64, false, 0);
            isSwordOut = true;
        ...
        if(onCollision(player, 16, 16, coin, 16, 16) && !coinCollected){
            soundPlaySample(pickup_bin, SoundFormat_16Bit, pickup_bin_size, freq, 64, 64, false, 0);
            coinCollected = true;
            coins++;
            coin->isHidden = true;
        }
        ...
        if(onCollision(sword, 16, 16, enemy, 16, 16)){
            soundPlaySample(hit_bin, SoundFormat_16Bit, hit_bin_size, freq, 100, 64, false, 0);
            enemy->x = -16;
        }
        if(onCollision(player, 16, 16, enemy, 16, 16)){
            player->x = 50;
            player->y = 50;
            coins -= 5;
            if(coins < 0) coins = 0;
        ...
        if(!jump){
            ...
            for(int i = 0; i < 5; i++){
                if(onCollision(player, 16, 16, grasses[i], 32, 32)){
                    player->y = grasses[i]->y-15;
                    yvel = 0;
                }
                if(onCollision(enemy, 16, 16, grasses[i], 32, 32)){
                    enemy->x += 2;
                }
            }
